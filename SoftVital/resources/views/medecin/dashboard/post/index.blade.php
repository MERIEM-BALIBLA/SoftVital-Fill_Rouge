            ...
            <div class="flex flex-col sm:flex-row gap-8">
                @foreach ($posts as $post)
                    <div class="relative bg-white md:w-1/3 w-full shadow rounded-lg overflow-hidden">
                        ...
                        <div class="p-6">
                            <span
                                class="block text-slate-400 font-semibold text-sm">{{ $post->created_at->format('Y-m-d') }}</span>
                            ...
                            <div class="flex mt-4 gap-4 items-center">
                                <span class="flex gap-1 items-center text-sm">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="text-sky-400 w-4 h-4">
                        ...
                        <form action="{{ route('destroy', ['post' => $post->id]) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button> <svg class="absolute top-1 right-1" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <a href="{{ route('edit', ['post' => $post->id]) }}">
                            <svg class="absolute bottom-1 right-1" xmlns="http://www.w3.org/2000/svg" width="2em"
                ...
                @endforeach
                <div class="relative bg-white md:w-1/3 w-fullshadow rounded-lg overflow-hidden">

                    <a href="{{ route('create') }}" class="">
                        {{-- <span class="relative text-black hover:text-white">ajouter un nouveuau poste</span> --}}
                        <svg xmlns="http://www.w3.org/2000/svg" width="4em" height="4em" viewBox="0 0 24 24">
                            <g fill="none" stroke="#dcdbdb" stroke-dasharray="18" stroke-dashoffset="18"
                                stroke-linecap="round" stroke-width="2.6">
                                <path d="M12 5V19">
                                    <animate fill="freeze" attributeName="stroke-dashoffset" begin="0.4s"
                                        dur="0.3s" values="18;0" />
                                </path>
                                <path d="M5 12H19">
                                    <animate fill="freeze" attributeName="stroke-dashoffset" dur="0.3s"
                                        values="18;0" />
